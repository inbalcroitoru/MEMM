...
class feature_statistics_class():
    ...
    def __init__(self):
        ...
        self.is_company_name_count_dict = OrderedDict()          #capture words that are company name
        ...
        with open(file_path) as f:
            for line in f:
                ...
                for word_idx in range(len(splited_words)):
                    ...
                    for neighbor_word, neighbor_count_dict in zip([prev_word, next_word], [self.prev_word_curr_tag_count_dict, self.next_word_curr_tag_count_dict]):
...
class feature2id_class():
    ...
    def __init__(self, feature_statistics, threshold):
        ...
        # Init all features dictionaries - matching dictionaries to the ones in feature_statistics_class
...
def calc_normalization_term(w_i, f_h_all_tags_dict, num_of_features):
    """
        ...
        :param f_h_all_tags_dict: dictionary with {keys: history, values: list of lists where each outer list fit to a history
            and each inner list is relevant features indexes for curr history and specific tag from all possible tags} 
		"param num_of_features: total number of features 
...
print("Total time of function fmin_l_bfgs_b is: ", end-start)
...
#---------------------------saving all dictionaries for the inference part--------------------------#
